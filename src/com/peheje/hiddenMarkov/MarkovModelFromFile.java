    ...
    public MarkovModelFromFile(String path) throws IOException {
        ...
        for (int i = 0; i < lines.size() - 1; i++) {
            ...
            if (l.contains("hidden")) {
                ...
                initial = Arrays.stream(lines.get(i + 1).split(" ")).map(Double::parseDouble).collect(Collectors.toList());
            } else if (l.contains("transitions")) {
                List<List<Double>> matrix = new ArrayList<>();
                for (int j = i + 1; j < lines.size() && lines.get(j).length() > 0; j++)
                    matrix.add(Arrays.stream(lines.get(j).split(" ")).map(Double::parseDouble).collect(Collectors.toList()));
                double[][] doubles = matrix.stream().map(x -> x.stream().mapToDouble(p -> p).toArray()).toArray(double[][]::new);
